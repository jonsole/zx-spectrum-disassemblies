...
def picture_blocks(memory) -> tuple[str, list[tuple[int, int]]]:
    ...
    def where(location: int) -> str:
        return (name_of(memory, rooms[location]["start"] + 2)
                if location in rooms else "?")
    out = ["; Generated by scripts/build_hobbit.py -- do not edit.", ""]
    out += [
    ...
    for location, start, address in records:
        # The location, and its picture's label in a DEFW.
        out.append(f"B ${address:04X},1,1")
        out.append(f"  ${address:04X},1 Location {location}, {where(location)}")
        out.append(f"W ${address + 1:04X},2,2")
    ...
    for i, (start, end, location, counts, visited) in enumerate(streams):
        ...
        label = f"LOC{location}_{_label_word(where(location))}_PIC"
        summary = ", ".join(f"{n} {k}{'s' if n != 1 else ''}"
                            for k, n in counts.items() if n)
        out.append(f"@ ${start:04X} label={label}")
        out.append(f"b ${start:04X} Picture for location {location}: {where(location)}")
        ...
        ending = (f"counting the part it shares with location {partner}, "
                  f"{where(partner) if partner else ''}, and "
...
def build_fast_draw(snapshot: Path) -> None:
    ...
    stray = [LOAD_ADDR + i for i, (a, b) in enumerate(zip(original, patched))
             if a != b and not any(lo <= LOAD_ADDR + i < hi for lo, hi in PATCH_RANGES)]
    if stray:
