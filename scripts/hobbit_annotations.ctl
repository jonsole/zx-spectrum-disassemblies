...
D $CACB The scripts themselves are in $C82D-$CA83, each table followed by its scripts, every step decoded and every address in them a label (generated by build_hobbit.py's script_blocks). An instruction's low four bits are its opcode: 0 to 3 as SCRIPT_DO, 4 as SCRIPT_BARE, $0C, $0E and $0F as CHARACTERS_ACT says, and anything else sends the character back to its first script. Bit 4 means a 2-byte fallback follows, bit 5 that the character leaves the story when the step succeeds, and bit 6 that an order cannot interrupt it.
