...
# The fast-draw recordings for the Patches page, each the original and the
...
def build(html_dir: Path, out_ref: Path) -> None:
    ...
    fast = []
    for name, caption in FAST_DRAW_GIFS:
        gif = bh.OUT_DIR / name
        if not gif.exists():
            continue
        (html_dir / "hobbit" / "videos").mkdir(parents=True, exist_ok=True)
        (html_dir / "hobbit" / "videos" / name).write_bytes(gif.read_bytes())
