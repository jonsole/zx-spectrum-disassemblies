...
FAST_DRAW_VIDEO = "bag_end_side_by_side.mp4"   # made by recording, not by the build
...
def build(html_dir: Path, out_ref: Path) -> None:
    ...
    # The fast-draw recording for the Patches page, if one has been made: the
    # opening of the game, original and patched side by side, recorded on the
    # emulator at real speed. It is copied into the site with the pictures;
    # without it the section is empty and the page reads the same less it.
    video = bh.OUT_DIR / FAST_DRAW_VIDEO
    fast = []
    if video.exists():
        (html_dir / "hobbit" / "videos").mkdir(parents=True, exist_ok=True)
        (html_dir / "hobbit" / "videos" / FAST_DRAW_VIDEO).write_bytes(video.read_bytes())
        fast = [f'<video src="../videos/{FAST_DRAW_VIDEO}" controls loop muted playsinline '
                'style="max-width: 100%; image-rendering: pixelated"></video>',
                '<p>Recorded on the emulator at real speed, from the title screen: the '
                'original on the left, patched on the right.</p>']
